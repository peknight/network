...
trait UsernamePasswordApi[F[_], Auth]:
  def usernamePassword(state: AuthRequiredMethodSelected[F]): F[UserPassword]
  def authenticated(state: UsernamePasswordAuthenticating[F]): F[Auth]
...
object UsernamePasswordApi:
  private case class UsernamePasswordApi[F[_]: Applicative, Auth](userPassword: UserPassword, auth: Auth)
    ...
    def usernamePassword(state: AuthRequiredMethodSelected[F]): F[UserPassword] = userPassword.pure[F]
    def authenticated(state: UsernamePasswordAuthenticating[F]): F[Auth] = auth.pure[F]
  end UsernamePasswordApi
