...
import com.peknight.socks5.Connection
import com.peknight.socks5.State.Initial
...
object Socks5Server:
  ...
  )(using Charset)(using Async[F], Network[F])
    extends com.peknight.socks5.server.io.Socks5Server[F, Auth, ConnectState, BindState, UDPAssociateState]:
  ...
  def apply[F[_], Auth, ConnectState, BindState, UDPAssociateState](
    api: Socks5ServerApi[F, Auth, ConnectState, BindState, UDPAssociateState],
    address: GenSocketAddress = SocketAddress.Wildcard,
    options: List[SocketOption] = Nil)(using Charset)(using Async[F], Network[F])
